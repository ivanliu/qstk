...
class DataAccess(object):
    ...
    def get_data_hardread(self, ts_list, symbol_list, data_item, verbose=False, bIncDelist=False):
        ...
        for sItem in data_item:
            ...
            if( self.source == DataSource.MLT or self.source == DataSource.YAHOO or self.source == DataSource.GOOGLE):
                if (sItem == DataItem.OPEN):
                    list_index.append(1)
                elif (sItem == DataItem.HIGH):
                    list_index.append (2)
        ...
        for symbol in symbol_list:
            ...
            try:
                if ( (self.source == DataSource.GOOGLE) or (self.source == DataSource.CUSTOM) or (self.source == DataSource.MLT) or (self.source == DataSource.YAHOO) ):
            ...
            if _file != None:
                if (self.source==DataSource.CUSTOM) or (self.source==DataSource.YAHOO) or (self.source==DataSource.GOOGLE)  or (self.source==DataSource.MLT):
